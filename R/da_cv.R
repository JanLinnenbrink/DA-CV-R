...
#'
#' @export
DA_CV <- function(samples, target, env_stack,
                  nodata_value, out_path, folds_k,
                  cate_num, autoc_threshold) {
  start_time <- Sys.time()
  ...
  terra::writeRaster(prob_raster,
                     file.path(out_path, "DA-CV_prob.tif"),
                     overwrite = TRUE)
  
  cate_raster <- prob_raster
  values(cate_raster) <- ifelse(values(prob_raster) >= threshold, 2, 1)
  terra::writeRaster(cate_raster,
                     file.path(out_path, "DA-CV_cate.tif"),
                     overwrite = TRUE)
