...
#' }
spatial_plus_cv <- function(samples, response_name,
                           cate_col_start = 0, cate_col_end = 0,
                           k = 5, sp_threshold = 1,
                           method = c("SP", "SP1", "SP2")) {
  ...
  if (start_cat > 0) {
    mat[, seq_len(start_cat)] <- apply(mat[, seq_len(start_cat), drop = FALSE], 2,
                                       function(col) (col - min(col)) / (max(col) - min(col)))
  }
  if (end_cat < ncol(mat)) {
    cols <- seq(end_cat + 1, ncol(mat))
    mat[, cols] <- apply(mat[, cols, drop = FALSE], 2,
                         function(col) (col - min(col)) / (max(col) - min(col)))
  }
  mat
}
  ...
  if (method == "SP1") sp_threshold <- sp_threshold
  if (method == "SP2") sp_threshold <- 0
  
  # --- Spatial clustering ---
  if (sp_threshold == 0) {
    clusters <- lapply(seq_len(nrow(pts_df)), function(i) list(ids = i))
  } else {
    model <- cluster::agnes(coords, diss = FALSE, 
  ...
  # --- Cluster averages ---
  norm_env <- normalize_cols(as.matrix(pts_df[, setdiff(names(pts_df), response_name), drop = FALSE]), cate_col_start, cate_col_end)
  ...
  ensemble_labels <- apply(cbind(coords_k, env_k, response_name_k), 1, 
                           function(x) as.numeric(names(sort(table(x), decreasing = TRUE)[1])))
  
  # --- Assign folds back to samples ---
  res <- do.call(rbind, lapply(seq_along(clusters), function(cid) {
